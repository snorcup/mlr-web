export class AudioEngine {
  ...
  makeTrack(index){ return {index, clipIndex:index, source:null, gain:null, playing:false, startedAt:0, offset:0, rate:1, loop:true, loopStart:0, loopEnd:null, volume:0.9, muted:false, once:false}; }
  async start(){
  ...
  playTrack(trackIndex, slice=0){
    ...
    const gain = new GainNode(this.context, {gain:track.muted?0:track.volume});
    const sliceOffset = (Math.max(0,Math.min(15,slice)) / 16) * clip.duration;
    // One-shot mode: no looping
    if(track.once){
      source.loop = false;
      source.loopStart = 0;
      source.loopEnd = 0;
  ...
  setVolume(trackIndex, value){ const track=this.tracks[trackIndex]; track.volume=Math.max(0,Math.min(1,Number(value))); if(track.gain && !track.muted) track.gain.gain.value=track.volume; }
  ...
  setMuted(trackIndex, muted){ const track=this.tracks[trackIndex]; track.muted=!!muted; if(track.gain) track.gain.gain.value=track.muted?0:track.volume; }
  toggleMute(trackIndex){ this.setMuted(trackIndex, !this.tracks[trackIndex].muted); return this.tracks[trackIndex].muted; }
  setOnce(trackIndex, once){ this.tracks[trackIndex].once=!!once; }
  positionSlice(trackIndex){
    ...
    if(track.once){
      // One-shot: position goes from slice to end of clip, then stops
      const pos = track.offset + elapsed;
